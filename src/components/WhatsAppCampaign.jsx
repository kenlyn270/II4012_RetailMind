...
export default function WhatsAppCampaign() {
  ...
  useEffect(() => {
    ...
    getDemoBlastTargets()
      .then((data) => { if (!cancelled) setDemoTargets(data.targets || []); })
      .catch(() => {});
  ...
  async function generateAIContent() {
    ...
    try {
      ...
      setError("AI generate gagal: " + err.message + ". Menggunakan template fallback.");
  ...
  async function handleSendTest() {
    if (!testNumber || !generatedMessage) return setError("Isi nomor test dan pesan dulu.");
    setLoading(true);
    setError("");
    try {
      const tempCampaign = campaign || await createDraftCampaign();
      setCampaign(tempCampaign);
      await testSend(tempCampaign.id, testNumber, generatedMessage.replaceAll("{cta_link}", ctaLink));
      setNotice("Test message berhasil diproses (dry-run jika token Fonnte belum diisi).");
    } catch (err) {
      setError(err.message);
    } finally {
      setLoading(false);
  ...
  async function launchCampaign() {
    ...
    try {
      ...
      const approved = draft.status === "draft" ? await approveCampaign(draft.id) : draft;
      const running = await triggerCampaign(approved.id);
      const jobData = await getCampaignJobs(running.id, { limit: 8 });
      setCampaign(running);
      setJobs(jobData.jobs || []);
      setStep(4);
      setNotice("Campaign berjalan. Worker akan mengirim bertahap sesuai throttle.");
    } catch (err) {
      setError(err.message);
    } finally {
      setLoading(false);
  ...
  async function handleDemoBlast() {
    ...
    try {
      ...
      setNotice(`Demo blast selesai: ${result.totalSent} terkirim, ${result.totalFailed} gagal${result.dryRun ? " (dry-run, FONNTE_TOKEN belum diisi)" : ""}.`);
    } catch (err) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-bold text-[#1C1D36]">WhatsApp Campaign</h2>
          <p className="text-slate-500 font-medium">Real audience preview, approval, test-send, and Fonnte dispatch.</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2 bg-emerald-50 px-4 py-2 rounded-full border border-emerald-100">
            ...
            <span className="text-xs font-bold text-emerald-700 uppercase tracking-wider">API Connected</span>
          </div>
        </div>
      </div>

      {(error || notice) && (
        <div className={`rounded-2xl px-4 py-3 text-sm font-semibold ${error ? "bg-red-50 text-red-600 border border-red-100" : "bg-emerald-50 text-emerald-700 border border-emerald-100"}`}>
          {error || notice}
        </div>
      )}

      <div className="flex items-center gap-2 mb-2">
        {[1, 2, 3, 4].map((s) => <div key={s} className="flex items-center"><div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs ${step === s ? "bg-[#FFD13B] text-[#1C1D36] shadow-md" : step > s ? "bg-emerald-500 text-white" : "bg-slate-200 text-slate-400"}`}>{step > s ? <Check className="w-4 h-4" /> : s}</div>{s < 4 && <div className={`w-8 md:w-16 h-0.5 mx-1 ${step > s ? "bg-emerald-500" : "bg-slate-200"}`} />}</div>)}
      </div>

      <div className="bg-white/70 backdrop-blur-md rounded-[32px] p-8 border border-white shadow-sm min-h-[520px] flex flex-col">
        {step === 1 && (
          <div>
            <h3 className="text-xl font-bold text-[#1C1D36] mb-6 flex items-center gap-2"><Users className="w-5 h-5 text-amber-500" /> Select Audience Segment</h3>
            {loading ? <Loader2 className="animate-spin" /> : <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {segments.map((seg) => {
                const Icon = iconMap[seg.id] || Users;
                return <button key={seg.id} onClick={() => handleSelectSegment(seg)} className="flex items-start gap-4 p-6 rounded-[24px] border-2 border-transparent bg-white hover:border-amber-300 hover:shadow-md transition text-left group">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${colorMap[seg.id] || "bg-slate-100 text-slate-600"}`}><Icon className="w-6 h-6" /></div>
                  <div className="flex-1"><div className="flex justify-between items-center mb-1"><p className="font-bold text-[#1C1D36]">{seg.label}</p><span className="text-xs font-bold px-2 py-0.5 rounded-lg bg-slate-100 text-slate-500">{statValue(seg.totalCustomers)} Customers</span></div><p className="text-xs text-slate-500 leading-relaxed">{seg.description}</p></div>
          ...
          <div>
            ...
            {loading || !preview ? <Loader2 className="animate-spin" /> : <>
              <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
                ...
                <div className="bg-emerald-50 rounded-2xl p-4"><p className="text-[10px] font-bold text-emerald-600 uppercase">Eligible</p><p className="text-2xl font-bold">{statValue(preview.eligibleContacts)}</p></div>
                <div className="bg-amber-50 rounded-2xl p-4"><p className="text-[10px] font-bold text-amber-600 uppercase">No Opt-in</p><p className="text-2xl font-bold">{statValue(preview.excluded.notOptedIn)}</p></div>
                <div className="bg-red-50 rounded-2xl p-4"><p className="text-[10px] font-bold text-red-600 uppercase">Blacklist</p><p className="text-2xl font-bold">{statValue(preview.excluded.blacklisted)}</p></div>
                <div className="bg-slate-50 rounded-2xl p-4"><p className="text-[10px] font-bold text-slate-400 uppercase">Freq Cap</p><p className="text-2xl font-bold">{statValue(preview.excluded.frequencyCapped)}</p></div>
              </div>
              <div className="bg-slate-50 rounded-2xl border border-slate-100 overflow-hidden mb-8"><table className="w-full text-left"><thead><tr className="border-b border-slate-200"><th className="p-4 text-[11px] font-bold uppercase text-slate-400">ID</th><th className="p-4 text-[11px] font-bold uppercase text-slate-400">Name</th><th className="p-4 text-[11px] font-bold uppercase text-slate-400">WA</th><th className="p-4 text-[11px] font-bold uppercase text-slate-400">Risk</th><th className="p-4 text-[11px] font-bold uppercase text-slate-400">Action</th></tr></thead><tbody className="divide-y divide-slate-100">{preview.sample.map((row) => <tr key={row.customerId}><td className="p-4 text-xs font-bold text-slate-500">{row.customerId}</td><td className="p-4 text-sm font-bold text-[#1C1D36]">{row.displayName}</td><td className="p-4 text-xs font-semibold text-slate-500">{row.phoneMasked}</td><td className="p-4 text-xs font-bold text-red-500">{row.churnRiskScore}%</td><td className="p-4 text-[10px] font-bold text-slate-500">{row.recommendedAction}</td></tr>)}</tbody></table></div>
              <div className="flex flex-col gap-3 mt-4">
                <input value={promoDetails} onChange={(e) => setPromoDetails(e.target.value)} className="w-full p-3 bg-slate-50 border border-slate-100 rounded-xl text-sm outline-none focus:border-amber-400" placeholder="Detail promo (opsional, misal: Diskon 15% semua produk)" />
                <button onClick={generateAIContent} disabled={isGenerating} className="w-full bg-[#1C1D36] text-white py-4 rounded-2xl font-bold flex items-center justify-center gap-3 hover:scale-[1.01] transition shadow-lg disabled:opacity-50">{isGenerating ? <><Loader2 className="w-5 h-5 animate-spin" /> Generating with AI...</> : <><Wand2 className="w-5 h-5 text-amber-400" /> Generate AI Copywriting (Gemini)</>}</button>
              </div>
            </>}
          </div>
        )}
        ...
        {step === 4 && campaign && (
          <div className="flex-1 flex flex-col items-center justify-center text-center p-8"><div className="w-24 h-24 bg-emerald-100 rounded-full flex items-center justify-center mb-6"><CheckCircle2 className="w-12 h-12 text-emerald-500" /></div><h3 className="text-3xl font-bold text-[#1C1D36] mb-2">Campaign {campaign.status}</h3><p className="text-slate-500 mb-6">Progress worker: {progress}% dari {statValue(totals.total)} jobs.</p><div className="grid grid-cols-4 gap-4 w-full max-w-2xl mb-6"><div><p className="text-2xl font-bold">{statValue(totals.queued)}</p><p className="text-[10px] uppercase font-bold text-slate-400">Queued</p></div><div><p className="text-2xl font-bold">{statValue(totals.sent)}</p><p className="text-[10px] uppercase font-bold text-slate-400">Sent</p></div><div><p className="text-2xl font-bold text-emerald-500">{statValue(Number(totals.delivered || 0) + Number(totals.read || 0))}</p><p className="text-[10px] uppercase font-bold text-slate-400">Delivered/Read</p></div><div><p className="text-2xl font-bold text-red-500">{statValue(totals.failed)}</p><p className="text-[10px] uppercase font-bold text-slate-400">Failed</p></div></div><button onClick={togglePause} className="bg-[#1C1D36] text-white px-6 py-3 rounded-full font-bold flex items-center gap-2">{campaign.status === "paused" ? <Play className="w-4 h-4" /> : <Pause className="w-4 h-4" />} {campaign.status === "paused" ? "Resume" : "Pause"}</button><div className="w-full max-w-3xl mt-8 text-left bg-slate-50 rounded-2xl overflow-hidden"><table className="w-full"><tbody>{jobs.map((job) => <tr key={job.id} className="border-b border-white"><td className="p-3 text-xs font-bold">{job.display_name}</td><td className="p-3 text-xs text-slate-500">{job.phone}</td><td className="p-3 text-xs font-bold">{job.status}</td></tr>)}</tbody></table></div></div>
        )}
      </div>

      {demoOpen && (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => !demoLoading && setDemoOpen(false)}>
          <div className="bg-white rounded-[32px] max-w-3xl w-full max-h-[90vh] overflow-y-auto p-8 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-start justify-between mb-6">
              <div>
                <h3 className="text-2xl font-bold text-[#1C1D36] flex items-center gap-2"><Zap className="w-6 h-6 text-amber-500" /> Demo Blast</h3>
                <p className="text-sm text-slate-500 mt-1">Kirim 1 pesan AI per segmen ke 4 nomor demo. AI copywriter berjalan per segmen.</p>
              </div>
              <button onClick={() => !demoLoading && setDemoOpen(false)} className="text-slate-400 hover:text-slate-600 font-bold text-xl">×</button>
            </div>

            <div className="bg-slate-50 rounded-2xl p-4 mb-4">
              <p className="text-[11px] font-bold uppercase text-slate-400 mb-3">Target Demo (4 Segmen K-Means)</p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                {demoTargets.map((t) => {
                  const Icon = iconMap[t.segmentId] || Users;
                  return (
                    <div key={t.segmentId} className="flex items-center gap-3 bg-white rounded-xl p-3 border border-slate-100">
                      <div className={`w-9 h-9 rounded-lg flex items-center justify-center ${colorMap[t.segmentId] || "bg-slate-100 text-slate-600"}`}><Icon className="w-4 h-4" /></div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-bold text-[#1C1D36] truncate">{t.segmentLabel}</p>
                        <p className="text-[11px] font-mono text-slate-500">{t.phoneMasked}</p>
                      </div>
                    </div>
                  );
                })}
            ...
            <div className="space-y-3 mb-5">
              ...
              <div>
                <label className="text-[11px] font-bold uppercase text-slate-400">Promo Details (opsional)</label>
                <input value={demoPromo} onChange={(e) => setDemoPromo(e.target.value)} disabled={demoLoading} className="w-full mt-1 p-3 bg-slate-50 border border-slate-100 rounded-xl text-sm outline-none focus:border-amber-400" placeholder="Misal: Diskon 20% untuk pembelian berikutnya" />
              </div>
            </div>
            ...
            >
              {demoLoading ? <><Loader2 className="w-5 h-5 animate-spin" /> Generating & Sending...</> : <><Send className="w-5 h-5" /> Generate AI + Blast ke 4 Nomor</>}
            </button>

            {demoResults && (
              <div className="space-y-3">
                <div className="flex items-center gap-3 text-sm font-bold">
                  <span className="px-3 py-1 rounded-full bg-emerald-100 text-emerald-700">✓ {demoResults.totalSent} sent</span>
                  {demoResults.totalFailed > 0 && <span className="px-3 py-1 rounded-full bg-red-100 text-red-600">✗ {demoResults.totalFailed} failed</span>}
                  {demoResults.dryRun && <span className="px-3 py-1 rounded-full bg-amber-100 text-amber-700">DRY-RUN</span>}
                </div>
                {demoResults.results.map((r) => (
                  <div key={r.segmentId} className={`rounded-2xl p-4 border ${r.ok ? "bg-emerald-50 border-emerald-100" : "bg-red-50 border-red-100"}`}>
                    <div className="flex items-center justify-between mb-2">
                      <p className="text-sm font-bold text-[#1C1D36]">{r.segmentLabel} → {r.phone}</p>
                      <span className="text-[10px] font-bold uppercase text-slate-500">{r.copySource || "error"}{r.copyModel ? ` · ${r.copyModel}` : ""}</span>
                    </div>
                    {r.ok ? (
                      <p className="text-xs text-slate-600 whitespace-pre-wrap leading-relaxed">{r.message}</p>
                    ) : (
                      <p className="text-xs text-red-600 font-semibold">{r.error}</p>
                    )}
                  </div>
                ))}
              </div>
